...
def create_datasets(files_dict, FLAGS, buffer_size):
    # Creates a dataset that reads all of the examples from two files, and extracts
    # the image and label features.    
    train_dataset = tf.data.TFRecordDataset(files_dict['train'])
    # train_dataset = train_dataset.shard(1, 0) # d = d.shard(FLAGS.num_workers, FLAGS.worker_index) when using paralell tensorflow!
    ...
    # train_dataset = train_dataset.interleave(tf.data.TFRecordDataset,cycle_length=4, block_length=FLAGS.batch_size)
    ...
    valid_dataset = valid_dataset.map((lambda x: _parse_function(x,'valid')), num_parallel_calls= 4)
    ...
    test_dataset = test_dataset.map(lambda x: _parse_function(x,'test'), num_parallel_calls= 4)
...
def get_files_dict(FLAGS):
    files_dict = {'train': glob.glob(os.path.join(PRO_DIR, FLAGS.train_dir, "*.tfrecords")), 
                  'valid': glob.glob(os.path.join(PRO_DIR, FLAGS.valid_dir, "*.tfrecords")), 
                  'test': glob.glob(os.path.join(PRO_DIR, FLAGS.test_dir, "*.tfrecords"))}
    return files_dict
...
def build_graph(architecture, FLAGS, files_dict):
    ...
    with tf.Graph().as_default() as comp_graph:
        ...
        train_dataset, valid_dataset, test_dataset = create_datasets(files_dict, FLAGS, suffle_size) #1000000, architecture['train_num_examples'])
