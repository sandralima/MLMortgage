...
def _parse_function(example_proto, tag):
  features = {tag+'/features': tf.FixedLenSequenceFeature([], tf.float32, default_value=0.0, allow_missing=True),
              tag+'/labels': tf.FixedLenSequenceFeature([], tf.int64, default_value=0, allow_missing=True)}
  parsed_features = tf.parse_single_example(example_proto, features)
  return parsed_features[tag+'/features'], parsed_features[tag+'/labels']


def create_datasets(files_dict, FLAGS, buffer_size, architecture):
    # Creates a dataset that reads all of the examples from two files, and extracts
    # the image and label features.    
    if (FLAGS.in_ram==True): #the datasets will be feeded from .h5 files:
        data = {}
        for key  in files_dict.keys():     
            data[key+'/features'] = np.empty((600000, architecture['n_input'])) #((architecture[key + '_num_examples'], architecture['n_input']))            
            data[key+'/labels'] = np.zeros((600000, architecture['n_classes'])) #((architecture[key + '_num_examples'], architecture['n_classes']))
            global_index = 0
            for file_path in files_dict[key]:    
                dataset_file = pd.HDFStore(file_path) # the first file of the path                
                dataset_features = dataset_file.select(key+'/features', start=0, stop=500000).values #, stop=500000
                nrows = dataset_features.shape[0] # dataset_file.get_storer(self.dtype + '/features').nrows        
                data[key+'/features'][global_index:global_index + nrows, :] = dataset_features
                print('features loaded')
                data[key+'/labels'][global_index:global_index + nrows, :] = dataset_file.select(key+'/labels', start=0, stop=nrows).values
                print('labels loaded')
                del dataset_features
                global_index += nrows
                # print(key, 'dataset_features: ', dataset_features.shape, 'dataset_labels', dataset_labels.shape)
                # data[key+'/features'] = np.concatenate((data[key+'/features'], dataset_features))                
                # data[key+'/labels'] = np.concatenate((data[key+'/labels'], dataset_labels))                 
                
                
        
        
        
        train_dataset = tf.data.Dataset.from_tensor_slices((data['train/features'].astype(np.float32), data['train/labels'].astype(np.int8)))
        train_dataset = train_dataset.repeat()  # Repeat the input indefinitely.
        train_dataset = train_dataset.shuffle(buffer_size=buffer_size, reshuffle_each_iteration=False)
        # train_dataset = train_dataset.map((lambda x: _parse_function(x,'train')), num_parallel_calls= 4) #FLAGS.num_map_threads)        
        train_dataset = train_dataset.batch(FLAGS.batch_size)

        valid_dataset = tf.data.Dataset.from_tensor_slices((data['valid/features'].astype(np.float32), data['valid/labels'].astype(np.int8)))
        # valid_dataset = valid_dataset.map(lambda x: _parse_function(x,'valid'))
        valid_dataset = valid_dataset.repeat()  # Repeat the input indefinitely.
        valid_dataset = valid_dataset.batch(FLAGS.valid_batch_size)
        
        test_dataset = tf.data.Dataset.from_tensor_slices((data['test/features'].astype(np.float32), data['test/labels'].astype(np.int8)))
        # test_dataset = test_dataset.map(lambda x: _parse_function(x,'test'))
        test_dataset = test_dataset.repeat()  # Repeat the input indefinitely.
        test_dataset = test_dataset.batch(FLAGS.test_batch_size)
        
    else:
        train_dataset = tf.data.TFRecordDataset(files_dict['train'])        
        train_dataset = train_dataset.repeat()  # Repeat the input indefinitely.
        train_dataset = train_dataset.shuffle(buffer_size=buffer_size)
        train_dataset = train_dataset.map(lambda x: _parse_function(x,'train'))
        train_dataset = train_dataset.batch(FLAGS.batch_size)
        
        
        valid_dataset = tf.data.TFRecordDataset(files_dict['valid'])
        valid_dataset = valid_dataset.map(lambda x: _parse_function(x,'valid'))
        valid_dataset = valid_dataset.repeat()  # Repeat the input indefinitely.
        valid_dataset = valid_dataset.batch(FLAGS.valid_batch_size)
        
        test_dataset = tf.data.TFRecordDataset(files_dict['test'])
        test_dataset = test_dataset.map(lambda x: _parse_function(x,'test'))
        test_dataset = test_dataset.repeat()  # Repeat the input indefinitely.
        test_dataset = test_dataset.batch(FLAGS.test_batch_size)
...
def nn_layer(input_tensor, output_dim, layer_name, FLAGS, act, train_flag, architecture):
    ...
    with tf.variable_scope(layer_name): # A context manager for defining ops that creates variables (layers).
        if (input_dim==None ): input_dim = architecture['n_input']
    ...
    with jit_scope(): #this operation will be compiled with XLA.
        for hid_i in range(1, FLAGS.n_hidden + 1):
            hidden_out = nn_layer(hidden_out,
                                  ...
                                  '{:1d}_hidden'.format(hid_i), FLAGS, act, train_flag, architecture)
    return hidden_out
...
#            features, center=True, scale=True, scope=scope)
    ...
    logits = (nn_layer(hidden_out, architecture['n_classes'],'9_softmax_linear', FLAGS, tf.identity, train_flag, architecture))    
    return logits
...
# ## GRAPH DEFINITION ## #
##########################
def build_graph(architecture, FLAGS, files_dict):
    ...
    with tf.Graph().as_default() as comp_graph:
        suffle_size = math.ceil(np.float32(architecture['train_num_examples'] * FLAGS.train_suffle_size)) 
        train_dataset, valid_dataset, test_dataset = create_datasets(files_dict, FLAGS, suffle_size, architecture) #1000000, architecture['train_num_examples'])
        iter = tf.data.Iterator.from_structure(train_dataset.output_types,
                                           train_dataset.output_shapes)
        features, labels = iter.get_next()
        train_init_op = iter.make_initializer(train_dataset, name='train_init_op')
        valid_init_op = iter.make_initializer(valid_dataset, name='valid_init_op')       
        test_init_op = iter.make_initializer(test_dataset, name='test_init_op')
        # print(train_init_op, valid_init_op, test_init_op)
...
# ## TRAINING & EVALUATION ## #
###############################
def run_model(comp_graph, name, net_number, FLAGS, architecture):
    ...
    with tf.Session(graph=comp_graph, config=config) as sess:
        ...
        try:
            ...
            return_epoch = batch_training(sess, writers, name, net_number, FLAGS, architecture)
            ...
            if FLAGS.test_flag:
                end_time = datetime.now() - start_time                                
                test_metrics = batching_dataset(sess, writers, 'test', architecture, FLAGS)
...
def acc_metrics_init(architecture):    
    acc_conf_mtx=np.zeros((architecture['n_classes'], architecture['n_classes']))
    acc_acc = 0
    acc_auc_list = np.zeros((architecture['n_classes']))
    acc_m_mtx = np.zeros((architecture['n_classes'], architecture['n_classes']))
    acc_loss = 0
    acc_log_loss = 0
    acc_auc_pr_list = np.zeros((architecture['n_classes']))
    epoch_metrics = (acc_conf_mtx, acc_acc, acc_auc_list, acc_m_mtx, acc_loss, acc_log_loss, acc_auc_pr_list)
    
    return epoch_metrics

def batching_dataset(sess, writers, tag, architecture, FLAGS):

    if tag =='valid':
        batch_num =   math.ceil(np.float32(architecture['valid_num_examples'] / FLAGS.valid_batch_size))  # 1000 #DATA.validation.total_num_batch(FLAGS.valid_batch_size) # 
        sess.run(['valid_init_op']) # switch to valid dataset                    
    else: #test
        batch_num = math.ceil(np.float32(architecture['test_num_examples'] / FLAGS.test_batch_size)) # 1000 # DATA.test.total_num_batch(FLAGS.test_batch_size) # 
        sess.run(['test_init_op']) # switch to test dataset                    
        
    metrics = acc_metrics_init(architecture)
    start_time = datetime.now()
    for batch_i in range(batch_num):
        print(tag, ':', batch_i)
        feed = create_feed_dict(tag, FLAGS)     
...
def batch_training(sess, writers, name, net_number, FLAGS, architecture):
    """Iterate over the dataset based on the number of epochs and train."""

    def train_one_epoch(epoch, batch_size, batch_num):
        
        epoch_metrics_train = acc_metrics_init(architecture)        
        try:
            print("Epoch: ", epoch)		
            ...
            for batch_i in range(batch_num):
                ...
                batch_dict= create_feed_dict('batch', FLAGS)
                step = epoch * batch_num + batch_i    # total steps for all epochs        
                if step > 0 and (step * batch_size) % (architecture['train_num_examples']) < batch_size:                
    ...
    try:
        ...
        if FLAGS.max_epoch_size==-1:
            batch_num = math.ceil(np.float32(architecture['train_num_examples'] / FLAGS.batch_size))        
        elif (FLAGS.max_epoch_size > architecture['train_num_examples']):
...
#            try:
            ...
            valid_metrics = batching_dataset(sess, writers, 'valid', architecture, FLAGS)
...
def create_feed_dict(tag, FLAGS):
    """Create the feed dictionary for mapping data onto placeholders in the graph."""
    if tag == 'batch':
        example_weights = np.array([1.0], dtype=np.dtype('float32'))
    elif tag == 'train':
        example_weights = np.array([1.0], dtype=np.dtype('float32'))
    elif tag == 'valid':
        example_weights = np.array([1.0], dtype=np.dtype('float32'))
...
def FLAGS_setting(FLAGS, net_number):
    ...
    FLAGS.valid_batch_size = 0.20
    FLAGS.test_batch_size = 0.20
    FLAGS.train_suffle_size = 1 # 0.10
    FLAGS.in_ram = True
...
def architecture_settings(files_dict, FLAGS):
    ...
    if (FLAGS.in_ram==True):
        ...
        architecture['train_num_examples'] = 19480053
        architecture['valid_num_examples'] = 1045164
        architecture['test_num_examples'] = 84185
    #    for key in files_dict.keys():
    #        total_records = 0
    #        for file in files_dict[key]:            
    #            total_records += sum(1 for _ in tf.python_io.tf_record_iterator(path=file))
    #        architecture[key + '_num_examples'] = total_records
        for string_record in  tf.python_io.tf_record_iterator(path=files_dict['train'][0]):
            example = tf.train.Example()
            example.ParseFromString(string_record)
            architecture['n_input'] = len(example.features.feature['train/features']
                                      .float_list
                                      .value) # array of float
        
            architecture['n_classes'] = len(example.features.feature['train/labels']
                                      .int64_list
                                      .value) # array of int
            break
